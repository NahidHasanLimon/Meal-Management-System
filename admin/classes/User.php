 ...
 $this->fm = new Format();
  ...
  public function admin_login($email,$password)
     ...
     {       $password=mysqli_real_escape_string($this->db->link,$password);
           ...
           Session::set("AdminUserName",$login_value['user_name']);
...
//    }
   //Update PRofile
   public function update_users($user_id,$name,$email,$about,$password)
   {
    ...
    $chekEmail="SELECT * FROM users WHERE id !='$user_id' AND email='$email'";
      $result= $this->db->select($chekEmail);
          if ($result !=false){
     ...
     public function add_users($name,$email,$about,$password)
         
   {
      ...
      $result= $this->db->select($chekEmail);
          ...
          if ($result!=false) {
                ...
                echo "This email Allready Taken";
              
    
    }
    //If Email Checking False
    else{

      $insert_user_query=" INSERT INTO users( name,email,password,about,created_at ) VALUES ('$name','$email','$password','$about',NOW() )";

      $insert_result=$this->db->insert($insert_user_query);
   ...
   {
      $query = " UPDATE users SET  status = CASE
    ...
    public function change_notice_status($notice_id)
         
   {
      $query = " UPDATE notice SET  status = CASE
                 WHEN status=0  THEN 1
                 WHEN status=1 THEN 0
                  END
       WHERE id = '$notice_id' ";
...
// }
   ...
   public function find_users_note($startDate)
   {
    $now = new DateTime($startDate);
    $start_date= $now->format('Y-m').'-01';

    $query = "SELECT * FROM users_note
    WHERE
    date >= '$start_date' + INTERVAL 0 MONTH AND date < '$start_date' + INTERVAL 1 MONTH
    ORDER BY date ASC ";
       $result=$this->db->class_select_option($query);
     return $result;
  ...
  a.`advanced_date` as advancedDate,a.id as id
   ...
   public function delete_rent_and_extra_costs($rent_costs_id)
      ...
      public function delete_users_note($users_note_id)
      {
        // print_r($payments_id);
       $query="DELETE FROM users_note WHERE   id='$users_note_id' ";
        $result = $this->db->delete($query);
        return $result;
      }
 ...
 public function delete_notice($notice_id)
   ...
   public function get_all_user_by_active_status()
   {
...
public function find_user_all_meal($user_id,$startDate)
   ...
   public function add_users_note($name,$subject,$description,$contact_no)
  {
   ...
   {
        $insert_query="INSERT INTO users_note(name,subject,description,date,contact_no) VALUES ('$name','$subject','$description',now(),$contact_no)";
   ...
   } public function add_notice($title,$description)
  {
   ...
   {
        $insert_query="INSERT INTO notice(title,description,date) VALUES ('$title','$description',now())";

          $results=$this->db->insert($insert_query);
          if ($results) {
              echo "Successfully Inserted Notice";

          exit();
          }
          else {
            echo "Failed to Insert Notice";
   ...
   public function  update_rent_and_extra_costs($rent_costs_id,$cost_type,$cost_date,$cost_amount)
  {
   ...
   } public function update_notice($notice_id,$title,$description)
  {


    if ($notice_id =="" || $title == "" || $description=="" ) {
      echo " Field Must Not be Empty";
   ...
   else
   {
        ...
        WHERE id = '$notice_id' ";
          ...
          if ($result) {
              echo "Successfully Updated Notice";
              exit();
          }
          else
          {
   ...
   public function find_all_notices($startDate)
   {
    ...
    $query= "SELECT * FROM notice  ORDER BY id ASC";
      ...
      $query= "SELECT * FROM notice WHERE date >= '$startDate' + INTERVAL 0 MONTH AND date < '$startDate' + INTERVAL 1 MONTH ORDER BY id ASC";
    }
